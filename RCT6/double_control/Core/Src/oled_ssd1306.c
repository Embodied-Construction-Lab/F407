...
static HAL_StatusTypeDef OledSsd1306_WriteCommand(uint8_t command)
{
  ...
  return HAL_I2C_Master_Transmit(oled_i2c, oled_address, data, sizeof(data), 50U);
}
...
static void OledSsd1306_GetGlyph(char character, uint8_t glyph[5])
{
  ...
  case 'X':
    ...
    break;
  case 'a':
    glyph[0] = 0x20; glyph[1] = 0x54; glyph[2] = 0x54;
    glyph[3] = 0x54; glyph[4] = 0x78;
    break;
  case 'd':
    glyph[0] = 0x38; glyph[1] = 0x44; glyph[2] = 0x44;
    glyph[3] = 0x48; glyph[4] = 0x7F;
    break;
  case 'e':
    glyph[0] = 0x38; glyph[1] = 0x54; glyph[2] = 0x54;
    glyph[3] = 0x54; glyph[4] = 0x18;
    break;
  case 'g':
    glyph[0] = 0x18; glyph[1] = 0xA4; glyph[2] = 0xA4;
    glyph[3] = 0xA4; glyph[4] = 0x7C;
    break;
  case 'n':
    glyph[0] = 0x7C; glyph[1] = 0x08; glyph[2] = 0x04;
    glyph[3] = 0x04; glyph[4] = 0x78;
    break;
  case 'r':
    glyph[0] = 0x7C; glyph[1] = 0x08; glyph[2] = 0x04;
    glyph[3] = 0x04; glyph[4] = 0x08;
    break;
  case 's':
    glyph[0] = 0x48; glyph[1] = 0x54; glyph[2] = 0x54;
    glyph[3] = 0x54; glyph[4] = 0x20;
    break;
  case 't':
    glyph[0] = 0x04; glyph[1] = 0x3F; glyph[2] = 0x44;
    glyph[3] = 0x40; glyph[4] = 0x20;
    break;
  case 'w':
    glyph[0] = 0x3C; glyph[1] = 0x40; glyph[2] = 0x30;
    glyph[3] = 0x40; glyph[4] = 0x3C;
    break;
  case 'x':
    glyph[0] = 0x44; glyph[1] = 0x28; glyph[2] = 0x10;
    glyph[3] = 0x28; glyph[4] = 0x44;
    break;
  default:
    break;
  }
...
void OledSsd1306_ClearRow(uint8_t row)
{
  if (row < OLED_SSD1306_ROWS)
  {
    memset(&oled_buffer[(uint16_t)row * OLED_SSD1306_WIDTH], 0,
           OLED_SSD1306_WIDTH);
  }
}
...
HAL_StatusTypeDef OledSsd1306_UpdateRow(uint8_t row)
{
  if ((oled_i2c == NULL) || (oled_address == 0U) ||
      (row >= OLED_SSD1306_ROWS))
  {
    return HAL_ERROR;
  }

  if ((OledSsd1306_WriteCommand((uint8_t)(0xB0U + row)) != HAL_OK) ||
      (OledSsd1306_WriteCommand(0x00U) != HAL_OK) ||
      (OledSsd1306_WriteCommand(0x10U) != HAL_OK))
  {
    return HAL_ERROR;
  }
  return OledSsd1306_WriteData(
      &oled_buffer[(uint16_t)row * OLED_SSD1306_WIDTH],
      OLED_SSD1306_WIDTH);
}
