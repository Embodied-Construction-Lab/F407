...
static float TruckControl_RawPedalToUnit(float raw_axis)
{
  const float denominator = TRUCK_RAW_PEDAL_RELEASED -
      TRUCK_RAW_PEDAL_PRESSED;
  float value;

  raw_axis = TruckControl_Clamp(raw_axis,
                                TRUCK_RAW_PEDAL_PRESSED,
                                TRUCK_RAW_PEDAL_RELEASED);
  value = (TRUCK_RAW_PEDAL_RELEASED - raw_axis) / denominator;
  if (value <= TRUCK_RAW_PEDAL_DEAD_ZONE)
  {
    return 0.0f;
  }
  return TruckControl_Clamp(value, 0.0f, 1.0f);
}

void TruckControl_NormalizeRawCommand(const TruckCommand *raw_command,
                                      TruckCommand *command)
{
  float forward;
  float reverse;

  if ((raw_command == NULL) || (command == NULL))
  {
    return;
  }

  forward = TruckControl_RawPedalToUnit(raw_command->throttle);
  reverse = TruckControl_RawPedalToUnit(raw_command->brake);
  command->steering = TruckControl_Clamp(
      raw_command->steering * TRUCK_RAW_STEERING_DIRECTION,
      -1.0f, 1.0f) * TRUCK_STEERING_MAX_DEG;
  ...
  {
    command->throttle = forward - reverse;
    command->brake = 0.0f;
  }
  command->up = raw_command->up;
  command->down = raw_command->down;
                             ...
                             TruckOutputs *outputs)
{
  ...
  {
    lift_speed = (float)TRUCK_LIFT_UP_MAX_PERCENT;
  }
  else if ((command->down != 0U) && (command->up == 0U))
  {
    lift_speed = -(float)TRUCK_LIFT_DOWN_MAX_PERCENT;
  }
  ...
  TruckControl_SetLiftPercent(
      outputs, (int16_t)((lift_speed >= 0.0f) ?
                         (lift_speed + 0.5f) :
                         (lift_speed - 0.5f)));
}
