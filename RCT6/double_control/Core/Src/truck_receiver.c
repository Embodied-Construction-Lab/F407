                                    ...
                                    uint8_t *value)
{
  ...
  {
    if ((numeric != 0.0f) && (numeric != 1.0f))
    {
      return false;
    }
    *value = (uint8_t)numeric;
    return true;
  }
...
static TruckCommandFormat TruckReceiver_GetFormat(const char *frame)
{
  const char *position = strstr(frame, "\"type\"");

  if (position == NULL)
  {
    return TRUCK_COMMAND_DIRECT;
  }
  ...
  if (strncmp(position, "logi_raw\"", 9U) == 0)
  {
    return TRUCK_COMMAND_LOGI_RAW;
  }
  if (strncmp(position, "button\"", 7U) == 0)
  {
    return TRUCK_COMMAND_BUTTON;
  }
  return TRUCK_COMMAND_DIRECT;
}
                                   ...
                                   uint8_t *field_mask)
{
  ...
  parsed.format = TruckReceiver_GetFormat(frame);
...
bool TruckReceiver_ParseJson(const char *frame, TruckCommand *command)
{
  ...
  if ((command == NULL) ||
      ...
      ((field_mask & (uint8_t)TRUCK_FIELD_CONTROL) !=
       (uint8_t)TRUCK_FIELD_CONTROL))
  {
    return false;
  }
  *command = parsed;
