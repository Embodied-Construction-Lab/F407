...
static void test_steering_throttle_and_brake_mapping(void)
{
  ...
  assert(outputs.pwm_count[TRUCK_CHANNEL_LIFT] == 307U);
  assert(outputs.pwm_count[TRUCK_CHANNEL_DRIVE] == 410U);
  assert(outputs.pwm_count[TRUCK_CHANNEL_UNUSED] == 0U);
  ...
  assert(outputs.brake_percent == 0);
  assert(outputs.drive_percent == -50);
  assert(outputs.pwm_count[TRUCK_CHANNEL_DRIVE] == 205U);
...
static void test_raw_wheel_normalization_and_lift(void)
{
  TruckCommand raw = {0};
  TruckCommand command = {0};
  TruckOutputs outputs;

  assert(TruckReceiver_ParseJson(
      "{\"type\":\"truck_control\",\"timestamp\":1.0,"
      "\"steering\":15.0,\"throttle\":1.0,\"brake\":0.0,"
      "\"up\":1,\"down\":0}",
      &command));
  TruckControl_MapCommand(&command, &outputs);
  assert(outputs.steering_deg == 15);
  assert(outputs.drive_percent == 50);
  assert(outputs.lift_percent == TRUCK_LIFT_UP_MAX_PERCENT);

  raw.format = TRUCK_COMMAND_LOGI_RAW;
  raw.steering = 0.5f;
  raw.throttle = -1.0f;
  raw.brake = 1.0f;
  raw.up = 1U;
  TruckControl_NormalizeRawCommand(&raw, &command);
  assert(command.format == TRUCK_COMMAND_DIRECT);
  assert(command.steering == 15.0f);
  assert(command.throttle == 1.0f);
  assert(command.brake == 0.0f);
  assert(command.up == 1U);
  TruckControl_MapCommand(&command, &outputs);
  assert(outputs.steering_deg == 15);
  assert(outputs.drive_percent == 50);
  assert(outputs.lift_percent == TRUCK_LIFT_UP_MAX_PERCENT);
  ...
  TruckControl_NormalizeRawCommand(&raw, &command);
  TruckControl_MapCommand(&command, &outputs);
  assert(command.throttle == 0.0f);
  assert(command.brake == 1.0f);
  assert(outputs.drive_percent == 0);
}
...
static void test_incomplete_json_is_rejected(void)
{
  ...
  assert(TruckReceiver_ParseJsonUpdate(
      "{\"type\":\"button\",\"up\":true,\"down\":false}",
      &update, &field_mask));
  assert(update.format == TRUCK_COMMAND_BUTTON);
  assert(field_mask == (TRUCK_FIELD_UP | TRUCK_FIELD_DOWN));
  assert(update.up == 1U);
  assert(update.down == 0U);

  assert(TruckReceiver_ParseJson(
      "{\"type\":\"logi_raw\",\"steering\":0.5,"
      "\"throttle\":-1,\"brake\":1,\"up\":1,\"down\":0}",
      &update));
  assert(update.format == TRUCK_COMMAND_LOGI_RAW);
  assert(update.up == 1U);
}
...
int main(void)
{
  ...
  test_raw_wheel_normalization_and_lift();
  ...
  puts("ZGT6 truck_control tests passed");
  return 0;
}
