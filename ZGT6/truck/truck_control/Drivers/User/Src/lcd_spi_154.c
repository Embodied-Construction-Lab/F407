...
static void LCD_SPI3_GpioInit(void)
{
...
static void LCD_SPI3_Init(void)
{
  ...
  LCD_SPI->CR1 = 0U;
  LCD_SPI->CR2 = 0U;
  LCD_SPI->CR1 = SPI_CR1_MSTR | SPI_CR1_SSM | SPI_CR1_SSI |
...
void  LCD_WriteCommand(uint8_t lcd_command)
{
	...
	while((LCD_SPI->SR & 0x0080) != RESET); 	// 先判断SPI是否空闲，等待通信完成
	
	LCD_DC_Command;	//	DC引脚输出低，代表写指令	

	LCD_SPI->DR = lcd_command;		// 发送数据
	while( (LCD_SPI->SR & 0x0002) == 0);		// 等待发送缓冲区清空
	while( (LCD_SPI->SR & 0x0080) != RESET);	//	等待通信完成
...
*
*	说    明:	1. 使用示例 LCD_SetAsciiFont(&ASCII_Font16) ，即设置 2412的 ASCII字体
...
*	函数功能:	在指定坐标显示指定的字符
*
*	说    明:	1. 可设置要显示的字体，例如使用 LCD_SetAsciiFont(&ASCII_Font16) 设置为 2412的ASCII字体
...
void LCD_SetTextFont(pFONT *fonts)
{
	...
	/* 本项目仅移植 ASCII 1608 字库，中文字体接口保留兼容性。 */
...
*	函数功能:	在指定坐标显示指定的整数变量
*
*	说    明:	1. 可设置要显示的字体，例如使用 LCD_SetAsciiFont(&ASCII_Font16) 设置为的ASCII字符字体
*					2.	可设置要显示的颜色，例如使用 LCD_SetColor(0x0000FF) 设置为蓝色
*					3. 可设置对应的背景色，例如使用 LCD_SetBackColor(0x000000) 设置为黑色的背景色
...
{  
	...
	{
		(void)snprintf(Number_Buffer, sizeof(Number_Buffer), "%0*ld",
		               (int)len, (long)number);	// 将 number 转换成字符串，便于显示		
	}
	else			// 多余位补空格
	{	
		(void)snprintf(Number_Buffer, sizeof(Number_Buffer), "%*ld",
		               (int)len, (long)number);	// 将 number 转换成字符串，便于显示		
	}
