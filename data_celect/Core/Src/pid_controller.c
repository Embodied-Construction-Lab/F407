...
void PidController_Init(PidController *controller, float kp, float ki,
                        float kd, float feedforward_gain)
{
...
void PidController_InitAsymmetricFeedforward(
    PidController *controller, float kp, float ki, float kd,
    float positive_feedforward_gain, float negative_feedforward_gain)
{
  if (controller == NULL)
  {
    return;
...
float PidController_Update(PidController *controller, float target_speed,
                           float measured_speed, float dt_s)
{
  float error;
  ...
  output = (target_speed * feedforward_gain) + (controller->kp * error) +
