...
static JoystickProfile joystick_profile;
static JoystickProfileSample latest_profile_sample;
...
static void ClearLatestProfileSample(void)
{
  latest_profile_sample.x1 = 0.0f;
  latest_profile_sample.x2 = 0.0f;
  latest_profile_sample.y1 = 0.0f;
  latest_profile_sample.y2 = 0.0f;
}
...
static void ServiceGeneratedJoystickControl(void)
{
  ...
  now_ms = HAL_GetTick();
  if ((uint32_t)(now_ms - last_generated_control_tick) <
      JOYSTICK_CONTROL_PERIOD_MS)
  {
    return;
  }
  ...
  if ((homing_complete == 0U) && (homing_failed == 0U))
  {
    homing_status = HomingController_Update(&homing_controller, now_ms,
                                            &safety_feedback,
                                            homing_feedback_valid);
    latest_profile_sample = homing_controller.sample;
    if (homing_status == HOMING_CONTROLLER_COMPLETE)
    {
      homing_complete = 1U;
      JoystickProfile_Init(&joystick_profile, now_ms);
    }
    else if (homing_status == HOMING_CONTROLLER_TIMEOUT)
    {
      homing_failed = 1U;
      ClearLatestProfileSample();
    }
  }
  else if (homing_failed == 0U)
  {
    SafetyLimits_GetJoystickProfileFeedback(joystick_profile.selected_axis,
                                            &safety_feedback,
                                            &profile_feedback);
    JoystickProfile_Update(&joystick_profile, &profile_feedback);
    latest_profile_sample = joystick_profile.sample;
  }
  else
  {
    ClearLatestProfileSample();
  ...
  SafetyLimits_Apply(&latest_profile_sample, &safety_feedback);

  JoystickServoMap_Compute(latest_profile_sample.x1,
                           latest_profile_sample.x2,
                           latest_profile_sample.y1,
                           latest_profile_sample.y2,
...
static void FillMotionTelemetry(MotionTelemetry *telemetry, uint32_t now_ms)
{
  ...
  telemetry->x1 = latest_profile_sample.x1;
  telemetry->x2 = latest_profile_sample.x2;
  telemetry->y1 = latest_profile_sample.y1;
  telemetry->y2 = latest_profile_sample.y2;
...
int main(void)
{
  ...
  homing_complete = 0U;
  homing_failed = 0U;
  JoystickProfile_Init(&joystick_profile, HAL_GetTick());
